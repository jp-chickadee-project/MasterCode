...
def main():
    ...
    while True:
        ...
        if read_byte == b'\x02':
            ...
            """ 
            "rfid": ID,
                    "feederID": # Might just hard-code location for each specific Pi. ie. Cliff. Maybe specific files for each location
                    "visitTimestamp": int(time.time()),
                    "temperature": 0, # might not have temp.. but its a possibility
                    "mass": 108, # Load cell data
                    "bandCombo":"#a0/V"
            """
